...
- **预处理机制**（可选优化方案）：AI自动识别页面截图，智能生成元数据，减少实时识别请求，提升执行效率
...
4. **数据流转**：
   - 完全本地部署：所有AI模型在手机本地运行
   - 混合部署（可选）：轻量级模型本地，大模型云端
...
- **操作规划**：LLM根据用例和屏幕状态自主决策操作

#### 2.2.4 设备控制层
- **屏幕捕获**：实时获取设备屏幕截图（MediaProjection API）
- **元素定位**：基于Vision-Language模型识别元素位置（纯视觉）
- **操作执行**：执行点击、滑动、输入等操作（纯坐标操作）
...
- **屏幕理解**：Vision-Language模型（Qwen-VL等，纯视觉）
- **操作执行**：纯坐标操作（基于VL模型识别的坐标）
- **OCR**：ML Kit Text Recognition（降级方案，可选）
...
#### 视觉模型 - 本地部署
- **Vision-Language模型**：Qwen-VL-Chat（INT8 量化，**主要能力**）⭐
- **OCR**：ML Kit Text Recognition（降级方案，可选）
- **目标检测**：YOLOv8 Nano（可选，用于UI元素检测）
