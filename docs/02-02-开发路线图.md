...
**短期（1-2周内）**：
- **优先级1**：设计统一接口（IVisionLanguageService）（1-2天）⭐ **立即开始**
  - [ ] 定义`IVisionLanguageService`接口
  - [ ] 重构现有本地实现为`LocalVisionLanguageService`
  - [ ] 设计`APIVisionLanguageService`接口和配置结构
- **优先级2**：实现API客户端（APIVisionLanguageService）（2-3天）⭐ **立即开始**
  - [ ] 实现`APIVisionLanguageService`类
  - [ ] 支持多种认证方式（API Key、Bearer Token等）
  - [ ] 实现图像压缩和上传
  - [ ] 实现错误处理和重试机制
  - [ ] 配置API服务（优先使用云端API进行快速验证）
- **优先级3**：实现统一管理器（1天）⭐ **立即开始**
  - [ ] 实现`VisionLanguageManager`统一管理器
  - [ ] 支持配置选择部署方式（LOCAL / API_CLOUD / API_LOCAL）
  - [ ] 实现自动降级机制（本地失败时降级到API）
  - [ ] 集成到现有代码

**中期（2-3周内）**：
- **优先级4**：系统集成（3-5天）
  - [ ] 更新ElementLocator，支持VL模型定位（使用统一接口）
  - [ ] 更新SemanticMatcher，支持VL模型语义匹配
  - [ ] 更新TestExecutor，集成VL模型到测试执行流程
  - [ ] 实现降级机制（VL失败时自动降级到OCR）
- **优先级5**：LLM集成（5-10天）
  - [ ] 选择LLM服务（优先使用API方式，快速验证）
  - [ ] 实现用例理解功能
  - [ ] 实现操作规划功能
  - [ ] 实现AI自主决策
  - [ ] 测试完整流程（VL + LLM）
